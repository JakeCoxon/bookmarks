  ...
  function morph(from, toHtml, options) {
    ...
    function patchNodeValue(from2, to) {
      ...
      if (from2.nodeValue !== value) {
        ...
        breakpoint("Change text node to: " + value);
      }
    }
    function patchAttributes(from2, to) {
      ...
      for (let i = toAttributes.length - 1; i >= 0; i--) {
        ...
        if (from2.getAttribute(name) !== value) {
          ...
          breakpoint(`Set [${name}] attribute to: "${value}"`);
        }
      }
    }
      ...
      while (currentTo) {
        ...
        if (!currentFrom) {
          if (toKey && domKeyHoldovers[toKey]) {
            ...
            breakpoint("Add element (from key)");
          } else {
            let added2 = addNodeTo(currentTo, from2) || {};
            breakpoint("Add element: " + (added2.outerHTML || added2.nodeValue));
        ...
        if (toKey !== domKey) {
          ...
          if (toKey && domKey) {
            ...
            if (domKeyNode) {
              ...
              breakpoint("Swap elements with keys");
              continue;
            }
          }
        }
    ...
    breakpoint();
    //@jakecoxon: modified this line
    const result = patch(from, toEl) || from;
    fromEl = void 0;
    toEl = void 0;
    return result;
  }
