...
export class PureStore<S, T> {
  ...
  update = (updater: ((e: T) => void) | Partial<T>) => {
    ...
    this.root.callbacks.forEach((callback: (s: S) => void) =>
      callback(this.root.rootState)
    );
  };

  storeFor = <X>(getter: (s: T) => X): PureStoreReact<T, X> =>
    new PureStoreReact(this, getter);
  updaterFor = <X extends object>(getter: (s: T) => X) =>
...
export const createBasicStore = <S>(state: S) =>
  new PureStore(null, (s: S) => s, state);

export class PureStoreReact<S, T> extends PureStore<S, T> {
  usePureStore(): readonly [
    T,
    (updater: Partial<T> | ((e: T) => void)) => void
  ];
  usePureStore<X extends object>(
    getter?: (s: T) => X
